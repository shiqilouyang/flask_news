...
class User(BaseModel, db.Model):
    ...
    is_admin = db.Column(db.Boolean, default=False)  # true 代表是管理员！
    ...
    # 当前用户所发布的新闻  new.user 查询的是当前新闻有没有关注的人！虚拟的字段！
    news_list = db.relationship('News', backref='user', lazy='dynamic')


    # 设置密码的
    # @property将方法提升为属性的  默认是getter方法-->获取数据的
    @property
    def password(self):
    ...
    # 不同的视图函数, 可能需要返回不同的内容, 因此可以再模型内部, 提供多种模型转字典的函数
    def to_dict(self):
        resp_dict = {
            "id": self.id,
